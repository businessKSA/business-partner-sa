...
export function contractHtml({ ref, clientName, clientCr = "", clientVat = "", service, lines = [], net, vat, total, vatRate = 15, leadTime = "", executor = "", today = "", subscription = null }) {
  ...
  // A monthly subscription with a success fee is a different bargain from a
  // one-off service: it renews, it can be cancelled, and it charges a share of
  // revenue the client collects later. A contract that omits all three while
  // the pricing page promises them is not the agreement either side made.
...
<table><thead><tr><th>البند</th><th style="text-align:center">الكمية</th><th style="text-align:left">القيمة</th></tr></thead><tbody>${rows}</tbody></table>` },
    { title: "المقابل المالي", html: `<p>${sub ? "قيمة الدفعة الأولى قبل الضريبة" : "الإجمالي قبل الضريبة"}: <b>${net} ﷼</b><br>
ضريبة القيمة المضافة (${vatRate}%): <b>${vat} ﷼</b><br>
<b>${sub ? "المستحق الآن" : "الإجمالي المستحق"}: ${total} ﷼</b> (شامل ضريبة القيمة المضافة).</p>
<p>تُصدر فاتورة ضريبية نظامية بهذه القيمة عبر نظام الفوترة المعتمد لدى الطرف الأول، ويُسدَّد المقابل إلكترونياً أو بالتحويل البنكي.</p>` },
  ...
  if (sub) {
    sections.push({ title: "مدة الاشتراك وتجديده وإنهاؤه", html: `<p>هذه الخدمة اشتراك شهري يبدأ من تاريخ سداد الدفعة الأولى.</p>
<ol>
 <li>يتجدد الاشتراك تلقائياً لمدد شهرية مماثلة بقيمة <b>${esc(String(sub.renewsAt))} ﷼</b> شهرياً (تُضاف عليها ضريبة القيمة المضافة)، ما لم يُخطر أحد الطرفين الآخر برغبته في عدم التجديد.</li>
 <li>لأي من الطرفين إنهاء الاشتراك بإشعار كتابي قبل سبعة أيام على الأقل من نهاية الشهر الجاري، ويسري الإنهاء من بداية الشهر التالي.</li>
 <li>لا يترتب على الإنهاء استرداد رسوم الشهر الجاري، ويستكمل الطرف الأول التزامات ذلك الشهر.</li>
</ol>` });
    sections.push({ title: "عمولة النجاح", html: pct ? `<ol>
 <li>تستحق للطرف الأول عمولة نجاح بنسبة <b>${esc(String(pct))}%</b> تُحتسب على الإيراد الذي حصّله الطرف الثاني <b>فعلياً</b> من الصفقات المشمولة الموضحة في البند التالي.</li>
 <li>لا تستحق العمولة على عقد موقّع لم يُحصّل، ولا على فاتورة صادرة لم تُدفع، ولا على أي مبلغ رُدّ إلى العميل النهائي.</li>
 <li>لا تستحق العمولة على إيراد من عملاء الطرف الثاني القائمين قبل بدء الاشتراك، ولا على فرص لم يولّدها الطرف الأول.</li>
 <li>تُستحق خلال خمسة عشر يوماً من تاريخ التحصيل، وتصدر بها فاتورة ضريبية مستقلة عن رسوم الاشتراك الشهري.</li>
</ol>` : `<p>لا تستحق على هذا الاشتراك أي عمولة مهما بلغ الإيراد المحصّل؛ فالمقابل هو الرسوم الشهرية الثابتة وحدها، في مقابل اقتصار نطاق العمل على التوليد والتأهيل وحجز الاجتماعات دون المشاركة في الإغلاق.</p>` });
    if (pct) sections.push({ title: "الصفقات المشمولة وإثبات مصدر الفرصة", html: `<ol>
 <li>الصفقة المشمولة هي كل صفقة مع جهة أدرجها الطرف الأول في نظام إدارة العلاقات وسجّل عليها أول تواصل قبل بدء التفاوض، ولم تكن عميلاً قائماً للطرف الثاني ولا على قائمة استثناء متفق عليها كتابة عند بدء الاشتراك.</li>
 <li>السجل المدوَّن في نظام إدارة العلاقات لدى الطرف الأول هو المرجع في إثبات مصدر الفرصة وتاريخ أول تواصل.</li>
 <li>تظل الصفقة مشمولة لمدة اثني عشر شهراً من تاريخ أول تواصل موثّق، ولو أُبرمت بعد انتهاء الاشتراك.</li>
 <li>للطرف الثاني الاعتراض كتابة على شمول أي صفقة خلال عشرة أيام من إدراجها، ويُبتّ الاعتراض بالرجوع إلى السجل المشار إليه أعلاه.</li>
</ol>` });
  } else {
    sections.push({ title: "مدة التنفيذ", html: `<p>${leadTime ? `يلتزم الطرف الأول بالتنفيذ خلال ${esc(leadTime)} من تاريخ سداد المقابل واستكمال العميل للمستندات المطلوبة.` : "يبدأ التنفيذ من تاريخ سداد المقابل واستكمال العميل للمستندات المطلوبة، وفق المدة المتفق عليها."}</p>` });
  }
  sections.push(
    { title: "التزامات العميل", html: `<ol>
 ...
 <li>الرد على طلبات الاستيفاء خلال مدة معقولة؛ ولا يُحتسب على الطرف الأول أي تأخير ناشئ عن تأخر العميل.</li>
    ...
    { title: "الرسوم الحكومية", html: `<p>ما لم يُنص صراحةً على خلاف ذلك في جدول البنود أعلاه، لا تشمل قيمة هذا العقد الرسوم الحكومية أو رسوم الجهات الرسمية، ويتحملها العميل عند استحقاقها.</p>` },
    { title: "السرية", html: `<p>يلتزم كل طرف بالحفاظ على سرية ما يطّلع عليه من بيانات الطرف الآخر، وعدم إفشائها لغير أغراض تنفيذ هذا العقد أو ما توجبه الأنظمة.</p>` },
    { title: "الإنهاء", html: `<p>لأي طرف إنهاء العقد بإشعار كتابي عند إخلال الطرف الآخر بالتزاماته وعدم تصحيح الإخلال خلال خمسة عشر يوماً من الإشعار. ويُسوّى المستحق عن الأعمال المنفَّذة فعلياً حتى تاريخ الإنهاء.</p>` },
    { title: "النظام الواجب التطبيق", html: `<p>يخضع هذا العقد لأنظمة المملكة العربية السعودية، وتختص الجهات القضائية المختصة بالمملكة بالفصل في أي نزاع ينشأ عنه.</p>` },
  );
...
${sections.map((sec, i) => `<h2>${ORDINALS[i] || (i + 1)}: ${sec.title}</h2>\n${sec.html}`).join("\n\n")}

(function () {
  "use strict";
  function isAr() { return document.documentElement.lang === "ar" || (window.BP && BP.lang === "ar"); }
  ...
  document.addEventListener("click", function (e) {
    var tab = e.target.closest(".pk-tab");
    if (tab) {
      var group = tab.getAttribute("data-group");
      document.querySelectorAll(".pk-tab").forEach(function (x) { x.classList.toggle("active", x === tab); });
      document.querySelectorAll(".pk-panel").forEach(function (x) { x.classList.toggle("active", x.getAttribute("data-panel") === group); });
      return;
    }
    ...
    set("po-package-label", (isAr() ? btn.getAttribute("data-name-ar") : btn.getAttribute("data-name-en")) + " — " + (btn.getAttribute("data-price") || ""));
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    if (form && window.BP && BP.cart) {
      form.addEventListener("submit", function (e) {
        ...
        setTimeout(function () { location.href = isAr() ? "/ar/checkout" : "/checkout"; }, 450);
      });
    }
    ...
    if (pb) {
      setTimeout(function () {
        ...
        var name = isAr() ? (pkg.nameAr || pkg.nameEn) : (pkg.nameEn || pkg.nameAr);
        var entity = pd.entityType === "company" ? t("Company", "شركة") : (pd.entityType === "establishment" ? t("Establishment", "مؤسسة") : "-");
        var addons = (pd.addons || []).length ? pd.addons.map(function (a) { return esc(isAr() ? (a.nameAr || a.nameEn) : (a.nameEn || a.nameAr)) + " — " + esc(a.price || ""); }).join("<br>") : t("No add-ons selected.", "لا توجد إضافات مختارة.");

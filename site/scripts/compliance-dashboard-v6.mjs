...
// ===== وضع التضمين =====
// اللوحة صفحة مستقلة، لكن بوابة الخدمات المشتركة تحمّلها داخل تبويب. كل ما
// توفّره البوابة أصلاً — شريط الموقع، الشعار، أزرار الحساب، بطاقة «افتح بوابة
// العميل»، التذييل والزر العائم — تكرارٌ في ذلك السياق، فيُسقط ويبقى المحتوى.
// العلامة تُوضع قبل أول رسم حتى لا يومض الرأس المكرر ثم يختفي.
...
/* الجداول العريضة تمرّر داخل صندوقها بدل أن تدفع الإطار جانبياً */
...
@media(max-width:640px){.bp6-embed .hero-copy{padding:18px!important}
  /* زرٌّ بـnowrap أعرض من إطار الجوال — يلتفّ بدل أن يدفع الصفحة */
...
function acts(){if(document.getElementById('bp6Acts'))return;var copy=document.querySelector('.hero-copy');if(!copy)return;
  ...
  var up=document.createElement('button');up.className='pri';up.textContent='\u{1F4E4} رفع الملفات';

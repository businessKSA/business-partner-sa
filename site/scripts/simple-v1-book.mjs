...
.sv1-bk-when b{font-family:var(--fm)}
...
.sv1-bk-done .when{font-family:var(--fm);font-size:17px;color:var(--ac);margin:14px 0 6px}
...
function drawWhen(){
 ...
 var b=document.createElement('b');b.textContent=pick.time+' · '+slotMin+' '+TX.mins;
...
function done(o){
 ...
 w.textContent=p.dow+' '+p.day+' '+p.mon+' · '+pick.time+' ('+TX.tz+')';box.appendChild(w);

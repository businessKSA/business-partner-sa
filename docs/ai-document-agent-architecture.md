...
| 8 Stamp & Signature | ✅ توقيع العميل وختم المنشأة يُركَّبان فعلياً في Word وExcel وPDF |
...
### كيف يوقّع العميل
داخل لوحة العميل: يوقّع بإصبعه أو بالفأرة على لوحة رسم، أو يرفع صورة توقيعه.
اللوحة تُنتج **PNG بخلفية شفافة**، فيجلس التوقيع فوق سطر النموذج بدل أن يخفيه
بمربع أبيض. الختم يُرفع كصورة بنفس الطريقة.

يُحفظان **مرة واحدة على مستوى المنشأة** (`organizations.signature_storage_key`
و`stamp_storage_key`) ويُعاد استخدامهما في كل طلب لاحق.

### شرط الموافقة
`organizations.signature_consent_at` بوابة صارمة: بلا تاريخ موافقة صريح لا
يُطبَّق التوقيع على أي مستند، مهما كانت إعدادات الطلب. والوكيل لا يوقّع من
تلقاء نفسه أبداً — نفس القاعدة التي تمنعه من الإجابة عن الإقرارات القانونية.
`doc_agent_requests.signature_mode = 'client_image'` هو ما يطلب التطبيق،
و`stamp_mode` ('auto' افتراضاً / 'off') يتحكم بالختم.

### أين يُوضَع
لا يُرسم شيء عشوائياً: الوكيل يبحث عن الحقل الذي **يطلب** علامة يدوية —
`signature|signatory|توقيع` للتوقيع و`stamp|seal|ختم` للختم — ويستبعد صراحةً
ما يطلب اسماً أو تاريخاً (`date|تاريخ|name|الاسم`)، فحقل «تاريخ التوقيع» يبقى
نصاً عادياً. وطلبات التعبئة نفسها مُوجَّهة ألّا تكتب نصاً في حقل توقيع أو ختم.

| الصيغة | آلية الوضع |
|---|---|
| PDF (AcroForm) | صورة XObject داخل مستطيل `/Rect` الذي يعرّفه النموذج نفسه، بتحديث تزايدي — البايتات الأصلية تبقى كما هي |
| Word (.docx) | `<w:drawing>` داخل الفقرة التي تحمل عنوان الحقل |
| Excel (.xlsx) | `oneCellAnchor` في الخلية المجاورة لعنوان الحقل |

النِّسَب محفوظة دائماً (`Math.min` على البعدين) — التوقيع الممطوط يبدو مزوّراً.
وشفافية PNG تُنقَل إلى PDF عبر `/SMask` مستقل يبنيه `api/_img.js` بلا أي مكتبة
خارجية.

### ما تم التحقق منه
- `npm test`: 25 اختباراً، منها فكّ ترميز PNG بالبكسل، وتمرير JPEG كما هو،
  والوضع في الحقل الصحيح بالنِّسَب الصحيحة عبر أنماط PDF الثلاثة، وأن صورة
  غير صالحة تُعيد ملف العميل **كما هو بالبايت** بدل إفساده.
- ملفات Word وExcel حقيقية (مبنية بـpython-docx وopenpyxl) وُقِّعت ثم أُعيد
  فتحها بالمكتبتين نفسيهما: الصور في الفقرات/الصفوف الصحيحة، والأنماط
  والأجزاء الأصلية سليمة. وPDF مُوقَّع فُتِح بـpypdf: الصور موجودة بـ`/SMask`
  وتُفَكّ كـRGBA.
- لوحة التوقيع جُرِّبت في Chromium حقيقي بضربة مؤشّر فعلية: أنتجت PNG شفافاً
  فكّه محرّك الخادم نفسه، ورُفِض الحفظ بلا موافقة.

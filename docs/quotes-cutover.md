...
**تحديث 2026-09-16 — قرار المالك: «bp-quotes يندمج مع الرئيسي وما يستخدم مرة
تانية».** أُنجزت من الخطة كل خطوة لا تحتاج قاعدة اللوحة: التبعية ٤ **انتهت**،
وآلة التبعيتين ٢ و٣ **جاهزة وتعمل**، وبقي ما لا أملكه — `DATABASE_URL` الخاص
باللوحة — وهو تشغيل سكربت واحد بيدك. التفصيل في «الحالة اليوم» أسفل الملف.
...
## الحالة اليوم (2026-09-16)

| التبعية | الحالة | أين |
|---|---|---|
| ٤ — `‏/api/live-catalog` | **انتهت** | تُخدَم من `site/assets/data` عبر `api/_quotes.js` (`handleLiveCatalog`)، والتمريرة إلى اللوحة حُذفت من `vercel.json` |
| ٢ — `‏/quotes/d/<token>` | **الآلة جاهزة**، تنتظر النقل | `api/_quotes.js` (`handleLegacyDoc`) + جدول `legacy_doc_links` في `db/schema.sql` |
| ٣ — جسر المستندات | **انقلب**: القاعدة أولاً واللوحة ثانياً | `api/requests.js` (`action=my-documents`) يقرأ `requests` عبر `clientDocuments()` ثم يدمج ما عند اللوحة |
| ١ — `‏/quotes/:path*` | باقية | لوحة الإدارة نفسها؛ تسقط بعد النقل |

### كيف يعمل رابط المستند أثناء النقل

`‏/quotes/d/<token>` صار يمرّ بالموقع الرئيسي أولاً (ترتيب التحويلات في
`vercel.json` يضعه قبل `‏/quotes/:path*`):

1. رمز معروف في `legacy_doc_links` → يُخدَم المستند من `requests` — عرض سعر
   أو عقد أو فاتورة، بتنسيق قابل للطباعة و`noindex`.
2. رمز غير معروف → **تحويل ٣٠٢ إلى اللوحة** ما دامت حيّة. فالنقل تدريجيّ بلا
   يوم انقطاع، ولا رابط يموت في منتصف الطريق.
3. بعد حذف اللوحة يُضبط `QUOTES_PANEL_RETIRED=1` في Vercel → صفحة ٤١٠ عربية
   تدلّ العميل على حسابه، لا 404 صامت.

### الخطوتان الباقيتان — كلتاهما بيدك

النقل يحتاج قاعدة اللوحة، ولا أطلب سرّها في محادثة. خطوتان منفصلتان عمداً
حتى لا يجتمع سرّان في عملية واحدة:

```bash
# ١) التصدير — يقرأ قاعدة اللوحة ويكتب ملفاً تراجعه بعينك
cd quotes && npm ci && npx prisma generate
npx tsx scripts/export-for-main.ts        # → quotes/.migrate/quotes-export.json

# ٢) بروفة على القاعدة المحلية أولاً (لا تمسّ الإنتاج)
cd .. && LOCAL_DB=1 node ops/quotes-import.mjs --file quotes/.migrate/quotes-export.json
LOCAL_DB=1 node ops/quotes-import.mjs --file quotes/.migrate/quotes-export.json --write

# ٣) الإنتاج، بعد مراجعة البروفة
node ops/quotes-import.mjs --file quotes/.migrate/quotes-export.json --write
```

الاستيراد جافّ بلا `--write`، ويُطابِق بـ`ref` و`token` فإعادة تشغيله
تُحدِّث ولا تُضاعف — نقل يُعاد نصفه خير من نقل يُخاف إعادته.

بعدها: افحص رابطاً واحداً على `businesspartner.sa/quotes/d/<token>` فإن ظهر
المستند من قاعدتنا (لا تحويل)، احذف المشروع من لوحة Vercel، اضبط
`QUOTES_PANEL_RETIRED=1`، ثم احذف `quotes/` و`‏/quotes/:path*` من المستودع —
**بهذا الترتيب**، لأن حذف جذر مشروع حيّ يُسقط كل بناء عند تهيئة الحاوية قبل
أن يُقرأ `ignoreCommand` أصلاً (هذا ما جرى مع `erp/`).

...
> ## ⚠️ حدث لاحقاً — اقرأ هذا قبل المتن
>
> **هذه الوثيقة لقطة بتاريخ إصدارها، ومتنها لم يُعدَّل.** ما تغيّر بعد 2026-09-15 مسجَّل هنا فقط؛ حيث يتعارض المتن مع هذه القائمة، **القائمة هي الصحيحة**.
>
> | التاريخ | ما تغيّر | يُبطل في المتن |
> |---|---|---|
> | 2026-09-20 | **٨٣٦ صفحة خدمة** وُحِّدت على تصميم Simple V1 (كانت على طبقة `bp-public-v4` منفصلة). حلّ محلّ `public-pages-redesign.mjs` سكربتُ `simple-v1-service-pages.mjs` في سلسلة البناء. | — |
> | 2026-09-23 | **المزوّدات البديلة حُذفت من الكود.** `api/chat.js` لم يعد فيه إلا `azure` و`baher-n8n`؛ و`voiceProviders()` في `api/_docread.js` لم يعد فيه إلا `azure`. Gemini وGroq وAnthropic وOpenAI وElevenLabs **لم تعد مساراً احتياطياً تلقائياً في المحادثة ولا في التفريغ الصوتي**. | §6 سطر «بدائل الذكاء … احتياط للمحادثة العامة» · §8 سطر «المحادثة العامة ما زالت تحتفظ بها كاحتياط تلقائي» |
> | 2026-09-23 | **أساس Azure Postgres:** `api/_azpg.js` — مترجم من نحو PostgREST إلى SQL عبر حزمة `pg`. **مكتوب وغير مستورد من أي مكان**؛ القاعدة الحيّة ما زالت Supabase بالكامل. ربطه ينتظر حسم أي من `#318`/`#319`/`#320` يُعتمد. | — (إضافة على §8) |
> | 2026-09-23 | **طلب الدمج صار `#323`.** الـ`#271` **دُمج فعلاً** في `master` يوم 2026-09-04 (١٩٥ كوميتاً، `merged_at = 2026-09-04T01:53:07Z`) — لم يُهمل. لكن الدفع استمر بعده ثلاثة أسابيع بلا طلب يتتبّعه، فتراكمت ٧١ كوميتاً خارج `master`. | §1 جدول البنية: «PR #271 هو الـPR الوحيد للعمل» |
> | 2026-09-23 | **تصحيح مدى سجلّ Git.** المتن يقول إن السجلّ «يبدأ في 2026-08-22» — كان ذلك أثر نسخة مبتورة (`shallow clone`). السجلّ الحقيقي **١٬٠٤٧ كوميتاً تبدأ في 2025-08-25**: موقع Astro (أغسطس–سبتمبر 2025)، ثم صفحة دخول السوق (مايو–يونيو 2026)، ثم إعادة البناء الكبرى (يوليو 2026 فصاعداً). أي تقرير يُبنى من هذا المستودع يجب أن يتحقّق أولاً: `test -f .git/shallow`. | §7 سطر «سجلّ Git لهذا المستودع يبدأ في 2026-08-22» |
>
> **عند التحديث التالي:** إمّا أن يُعاد إصدار الوثيقة كاملةً بتاريخ جديد (ويُحذف هذا القسم)، أو يُضاف سطر هنا. لا يُعدَّل المتن في مكانه — فيفقد كونه لقطةً يُوثَق بتاريخها.

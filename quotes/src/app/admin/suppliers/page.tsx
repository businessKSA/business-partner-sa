  ...
  return (
    <>
          ...
          <tbody>
            {requests.map((r) => (
              <tr key={r.id}>
                ...
                <td>{r.titleAr}</td>
                <td><span className="pill">{SUPPLY_STATUS_LABEL[r.status]?.ar ?? r.status}</span></td>
      ...
      <div className="card">
        ...
        <table>
          <thead><tr><th>المورد</th><th>السجل التجاري</th><th>النشاط</th><th>الآيبان</th><th>التواصل</th></tr></thead>
          <tbody>
            {suppliers.map((s) => (
              <tr key={s.id}>
            ...
            {!suppliers.length ? <tr><td colSpan={5} className="muted">لا يوجد موردون.</td></tr> : null}
          </tbody>
        </table>
      </div>
    </>

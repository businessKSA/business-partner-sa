...
- **سباق الرايات (أونلاين):** نمط جماعي حيّ عبر قدرة الغرفة في صفحات claude.ai: اللاعبون يظهرون معاً بالاسم واللون، خمس رايات تُقبض بالضغط المطوّل، جولة ثلاث دقائق ولوحة نتائج. التفاصيل في `FORTNITE.md`.
- **الجِمال:** اقترب من جمل واضغط E لتركبه، Shift للهرولة، Space للنزول. لا يتسلّق ولا يدخل الأزقة الضيقة.
- **السلاح:** سيف في ١٧٤٤–١٨٢٤ وبندقية من ١٩٠٢، الهجوم بـ F يُسقط الحارس أمامك ثماني ثوانٍ بلا قتل؛ الحرّاس المطاردون يضربونك (ثلاثة قلوب) ثم تعود لآخر نقطة حفظ.
- **الكاميرا:** قفل المؤشر بالنقر، محور رأسي طبيعي مع خيار العكس، تقريب بالعجلة أو قرص الأصابع أو زرّي +/−، تصادم مع الجدران، وقصور ذاتي في الحركة.
- **الجوال:** بالعرض إلزامياً (شاشة إرشاد عند الوضع الرأسي)، ملء الشاشة عند البدء، إعدادات أداء أخفّ (بلا MSAA، ظلال أصغر، نخيل وغبار أقل).
...
- **الصوت والموسيقى:** نظام موسيقى بطبقات مع تلاشٍ متبادل: لحن رئيسي (`audio/theme.mp3`، مولّد عبر ElevenLabs Music: عود على مقام الحجاز ثم ناي ودفوف ووتريات) للقائمة والمشاهد ونهايات الفصول، وطبقة تسلّل أثناء اللعب، وطبقة مطاردة عند الإنذار. الطبقتان الأخيرتان تُبدَّلان تلقائياً ببديل تركيبي (WebAudio: عود ودُفّ ودرون على الحجاز بثلاث سرعات: هادئ/متوتر/مطاردة) إن غاب الملف.
- **السرد الصوتي:** تعليق عربي بصوت سعودي رزين (ElevenLabs، صوت «Ali») في مقدّمات المراحل ١ و٢ و٤ (`audio/narr-N.mp3`)، ويتوقف عند التخطي. المراحل ٣ و٥ و٦ نصية حتى تُولَّد ملفاتها (النصوص جاهزة في `LEVELS[i].intro`).
- مؤثرات تركيبية بلا ملفات: رياح، خطوات، نخرة تسلّق، نبض عند الإنذار، تنبيهات. منزلقات الموسيقى والمؤثرات في قائمة الإيقاف، و«M» للكتم.

### لإكمال الملفات الصوتية الناقصة
ضع في `game/audio/`: `stealth.mp3` (حلقة تسلّل هادئة ٦٠ث)، `chase.mp3` (مطاردة ٤٥ث)، `amb-day.mp3` و`amb-night.mp3` (أجواء)، و`narr-3.mp3`، `narr-5.mp3`، `narr-6.mp3` (نصوصها في الملف). تلتقطها اللعبة تلقائياً بلا تعديل في الكود.
